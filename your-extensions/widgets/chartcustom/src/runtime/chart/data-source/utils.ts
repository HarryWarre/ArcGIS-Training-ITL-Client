...
export const getSourceRecords = (
  ...
  const filteredRecords = records?.filter(
    (record) => record?.getData()?.[categoryField] != null
  );
  const rs = filteredRecords
    ?.map((record, i) => {
      if (dataSource && record.dataSource !== dataSource) {
        ...
        const objectid =
          categoryField === idField ? attributes[idField] ?? i : i;
 ...
 * @param series
 */
export const getInlineRecordslimited = (
  series,
 ...
 */
export const convertByFieldRecords = (
...
export const queryFieldUniqueValues = async (
...
export const useMemoizedQuery = (
  inputQuery: IMFeatureLayerQueryParams = DefaultQuery
) => {
  const { groupByFieldsForStatistics, outFields, outStatistics, where } =
    inputQuery;
...
export const updateDataSourceJson = (
  dsId: string,
  dsJson: ImmutableObject<DataSourceJson>
) => {
