...
import { useSelection, normalizeSeries } from "../utils";
...
function WithInlineDataChart(
  ...
  const [inlineData, dataItems] = React.useMemo(
    () => convertRecordsToInlineData(type, records, query, propSeries, intl),
    [type, records, query, propSeries, intl]
  );

  let series = React.useMemo(
    () => normalizeSeries(propSeries, query),
    [propSeries, query]
  );
  series = React.useMemo(
    () => normalizePieSlices(series, query, dataItems),
    [dataItems, query, series]
  );
  series = React.useMemo(
    () => normalizeSeriesName(series, query, translate),
    [series, query, translate]
  );
  webChart = React.useMemo(
    () => webChart.set("series", series),
    [webChart, series]
  );
  ...
  const chartWillRender: PreRenderCallback = React.useCallback(
    async (props) => {
      ...
      if (slices?.length) {
        slices = slices.map((slice) => {
          ...
          const usedSliceId = usedSlices?.find(
            (usedSlice) => usedSlice.originalLabel === id
          )?.sliceId;
          if (usedSliceId) {
            return {
              ...slice,
