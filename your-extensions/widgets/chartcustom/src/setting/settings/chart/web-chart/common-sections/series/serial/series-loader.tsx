...
const loadSplitByValues = async (
  ...
  return dataSource
    .query(params, { scope: QueryScope.InConfigView })
    .then((result) => {
...
export const SeriesLoader = (props: SeriesLoaderProps): React.ReactElement => {
  ...
  const handleLoadClick = async () => {
    try {
      setLoading(true);
      const inputValues = propValues.filter(
        (value) => value !== SplitByOtherSeriesValue
      );
      let values = await loadSplitByValues(
        dataSource,
        splitByField,
        inputValues
      );
      ...
      if (state) {
        meessageRef.current =
          state === "loadout"
            ? translate("allSeriesLoaded")
            : translate("tooManySeries");
        setVersion((v) => v + 1);
      }
      if (state === "exceed") {
  ...
  return (
    <>
      <Tooltip
        placement='left'
        title={translate("loadMoreSeries")}
        showArrow
        enterDelay={300}>
        <Button
          aria-label={translate("loadMoreSeries")}
          className={classNames("series-loader", className)}
          size='sm'
          icon
