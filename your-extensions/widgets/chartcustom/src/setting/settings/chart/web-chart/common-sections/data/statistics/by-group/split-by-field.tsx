import { type ImmutableArray, React, classNames, type QueriableDataSource, DataSourceManager, type UseDataSource, QueryScope, Immutable, hooks } from 'jimu-core'
...
interface Props {
  ...
  disabled?: boolean
  onChange?: (field: string, values: ImmutableArray<number | string>) => void
}
...
export const SplitByField = (props: Props): React.ReactElement => {
  ...
  const handleChange = async (fields: ImmutableArray<string>): Promise<void> => {
    ...
    if (splitByField) {
      try {
        ...
        if (!values.length) {
          ...
          onChange(splitByField, Immutable(values))
        }
        setLoading(false)
      } catch (error) {
        setLoading(false)
      ...
      onChange(splitByField, Immutable([]))
    }
  }
